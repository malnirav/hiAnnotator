...
getUCSCtable <- function(tableName, trackName, bsession=NULL, 
    ...
    if(!tableName %in% tableNames(ucscTableQuery(bsession,track=trackName))) {
        stop(paste("The provided table name:",tableName,
...
getRelevantCol <- function(col.names, col.options, 
    ...
    answer <- sapply(col.options, function(x) grep(x, col.names, 
                                                   ignore.case=TRUE))
...
#' This parameter is generally used to populate \code{\link{seqinfo}} slot of 
#' GRanges objects.
    ...
    if(!dists.only) {    
        ...
        res <- foreach(x=iter(res), y=iter(sapply(chunks,"[[","subject")), 
                       .inorder=FALSE, .combine=rbind) %dopar% {
                           ...
                           # use tapply instead of ddply() or by() because 
                           # it's a lot faster on larger datasets
                           bore <- with(x, 
                                        sapply(tapply(featureName,queryHits,
                                                      unique), 
                                               paste, collapse=","))
                           x$featureName <- 
                               as.character(bore[as.character(x$queryHits)])
                           
                           bore <- with(x, 
                                        sapply(tapply(strand,queryHits,unique), 
                                               paste, collapse=","))
                           x$strand <- 
                               as.character(bore[as.character(x$queryHits)])
                         ...
                         by(res.nrst, queryHits, function(n) 
                             with(n, 
                                  data.frame(queryHits=unique(queryHits), 
                                             ...
                                             dist=unique(dist), 
                                             featureName=
                                                 paste(unique(featureName),
                                                       collapse=","),
                                             strand=paste(unique(strand),
                                                          collapse=","),
                       ...
                       .packages=c("GenomicRanges","plyr")) %dopar% {                        
                           ...
                           counts <- sapply(widths, function(y) {
                               ...
                               if (weighted) {
                                   ...
                                   bore <- 
                                     mcols(x$subject)$weights[res.x$subjectHits]
                                   res.x$weights <- bore
                                   bore <- 
                                       mcols(x$query)$tempyID[res.x$queryHits]
                                   res.x$tempyID <- bore
                                   res.x$counts <- 
                                       with(res.x, ave(weights,tempyID,FUN=sum))
                                   res.x <- unique(res.x[,c("tempyID",
                                                            "counts")])
                                   nohits <- 
                                       setdiff(mcols(x$query)$tempyID,
                                               res.x$tempyID)
                                   res.x <- rbind(res.x, 
                                                  data.frame(tempyID=nohits,
                                                             counts=0))
    ...
    res <- foreach(x=iter(chunks), .inorder=FALSE, .combine=rbind,
                   ...
                   .packages=c("GenomicRanges","plyr")) %dopar% {                        
                       ...
                       if (asBool) {
                           ...
                           bore <- with(res.x, sapply(tapply(strand,queryHits,
                                                             unique), 
                                                      paste, collapse=","))
                           res.x$strand <- 
                               as.character(bore[as.character(res.x$queryHits)])
...
doAnnotation <- function(annotType=NULL, ..., postProcessFun=NULL, 
    ...
    if(is.null(annotType)) {
        stop("Please define the annotType parameter to identify which type of 
...
.checkArgsSetDefaults <- function() {
    ...
    checks <- expression(    
        ...
        if (!any(unique(as.character(seqnames(sites.rd))) %in% 
            ...
            stop("There are no seqnames/chromosomes that are shared between the 
